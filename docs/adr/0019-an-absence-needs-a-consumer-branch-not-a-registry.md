...
🔴 **防蔓延闸——「已被承认的欠账」不是「合法的缺席」。** `evals/` 有 `docs/checks.md` 的「尚未实现」一节兜着，但「13 条检查一条都没跑过」**不是合法状态**：它是丙，只是有票。（⚠️ 本句的现状陈述已过期：[#75](https://github.com/jiangxidong/EduApplication/issues/75)／[#76](https://github.com/jiangxidong/EduApplication/issues/76) 后已有 6 条在 CI 每次 push 跑——原句保留作当时的分类样本，闸的判据不变，就地标明照 ADR 0008 弱表述先例。）少这一句，任何人都能靠补一句「尚未实现」把丙洗成甲。这与 ADR 0008 拒绝把 `channels/` 定成「例外」是同一条理由：**没有判据的出口会蔓延。**
