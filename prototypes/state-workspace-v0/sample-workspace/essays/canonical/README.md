...
| `render_form` | 文件 | 存在的理由 |
|---|---|---|
| `long` | `long.md` | 信息量最大的一侧，短版本从它裁、**不反过来** |
| `short-250` | `short-250.md` | **不是长文的摘要**，是独立成篇的 250 词版本——摘要读起来像目录 |
| `points` | `points.md` | 短答题结构（如 UIUC 4×250）要的是互相独立的回答，从长文自动截会得到四段互相重复、且都缺头缺尾的碎片 |

有哪几篇、各是什么形态，`ls` 与 frontmatter 一看就知道，**不在这里记**。
