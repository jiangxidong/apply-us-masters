...
本文件的候选人三位**全部 `❌ 未确认`**，更谈不上起草授权，所以：

- **写入闸**（准备包阶段）：创建前必须先读到本文件里的授权声明——现在读不到，创建不了；
- **存在性闸**（入口阶段 / [#15](https://github.com/jiangxidong/EduApplication/issues/15)）：
  发现目录存在但本文件无授权声明 → **报错**。目录缺席时这道闸静默通过，是正确行为。

授权声明将来落在本文件（第四块「起草授权」）。**目录的缺席本身是信息**，别当成漏填去补齐。
