...
⚠️ **样例里这一层是空的，但样例没有违规——缺的是契约。**
三个文件 `grep -c '^#### '` 全为 0：`### <program_key>` 之下直接就是行，共 9 行，**全部是 `待核实` 占位行**。
[ADR 0008](../../docs/adr/0008-the-owner-binds-to-a-section-not-a-file.md) 的第 6 条检查是
「`#### <类别>` **必嵌在**存在的 `### <program_key>` 下」——它只约束 `####` 的**位置**，
**没有反过来要求 `###` 之下必须有 `####`**。零个 `####`，这条检查空过，不报警。

真正的洞在本节：**owner 阶段尚未写入时，`### <program_key>` 之下该长什么样，没规定。**
裸行合不合法？要求先有 `#### <法定节名>` 又与上面「只建自己 owner 的节、不预建空节」直接打架。
→ [#28](https://github.com/jiangxidong/EduApplication/issues/28)，**修的是这段话，不是给样例补 `####`**。
...
> **是有意的**，验收时别当成漏填去补齐。
>
> 🔴 **但样例过不了上面这条前缀匹配，六个节匹配失败**（实测）。坏的不是后缀，是**前缀**——
> `## 🔴 推荐信机制 —— 风险 A：冒名发信` 的 `🔴 ` 顶在节名**之前**，`index(标题, "推荐信机制") == 1` 当场不成立。
> 三个文件的 `推荐信机制` 无一幸免，另有两个 `冻结点`、一个 `硬约束`。
> **[#23](https://github.com/jiangxidong/EduApplication/issues/23) 的结论、[#24](https://github.com/jiangxidong/EduApplication/issues/24)
> 的票面、本节此前的验收注三处都断言「样例全部过得了前缀匹配」，三处都是错的。**
> 是移走 `🔴` 还是放宽判据 → [#28](https://github.com/jiangxidong/EduApplication/issues/28)，本节不就地拍板。
