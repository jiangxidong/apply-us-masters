...
|---|---|---|---|
| `apply.md` | frontmatter + 正文 | **工作区标识 + 申请季** | 冷启动阶段创建；换季时唯一被改的地方 |
...
所以 `programs.tsv` 只放**取证能落在同一页上的少量字段**（deadline / 学费口径 / 门槛），
其余事实进 `channels/` 文件，在那里逐行带自己的标记。

> ⚠️ 这条是本原型最需要人拍板的地方，见下面 fork 3。
...
### fork 3 — 「二元 + 一列」在一行有多个事实时怎么办？

地图已锁定「二元，仅项目池表格一列」。原型的实现是**行级合取**：行内任一事实未被 evidence URL 覆盖 → 整行 `待核实`。
后果：deadline 已核实但学费没核实的行，会整行显示 `待核实`，**看不出已经核实了一半**。
原型的对策是让 `programs.tsv` 只放能落在同一页上的少量字段。
**如果你希望表里字段更多，这条锁定的决策需要重开。**
