...
> 数据取自 2026-08 的调研（#6 原始调研 + `research/country-delta` 分支补的语言小分与申请费），**演示用，本季必须重查**。
> 本文件申请人无关部分与 `prototype/state-layer` 的 `sample-workspace/channels/cornell--gradschool.md` 一致——
> P1 复用它作申请人侧底本；三 persona 共享同一份（#87）。
...
- `✓ https://gradschool.cornell.edu/admissions/application-steps/required-tests/` **官方送分机构代码 = 2098**：
  ...
  → 这条不是费用，是**第三方（ETS）把一份材料送到学校的渠道地址**，与上面的 IIE / Amideast / SOPHAS 同形状
...
- `✓ https://gradschool.cornell.edu/admissions/application-steps/pay-fees/` 申请费 **$105**，国际生同价：`"The nonrefundable application fee is $105."` ／ `"International applicants pay the same $105 application fee as U.S. applicants"`
...
- `待核实` GRE 是否要求
- `✓ https://gradschool.cornell.edu/admissions/application-steps/required-tests/english-language-proficiency-requirement-2/` **单项分数线（2026-01-21 前量表）**：`Speaking: 22 或以上 / Reading: 20 / Listening: 15 / Writing: 20`；IELTS `An overall band score of at least 7.0`
  → **本渠道池内唯一有语言小分**的项目——四项分开卡线，不像 Columbia/UIUC 那样只看总分
- `✓ 同上` 另设 **Score Tier A / Tier B** 两档，`Tier A`（IELTS 7.0 且口语 8.0 以上）**免额外英语评估即可当 TA**——这一档与本项目录取门槛不是同一件事，本工作区不据此判分档，只记录

#### 费用与资格

- `待核实（官网未列）` CIP code（用于判 STEM OPT 资格；STEM 归属是派生视图，渲染时对照 DHS 名单现算，不落盘）
  → 已实际取回 `https://catalog.cornell.edu/programs/computer-science-cscn-meng/`、`https://irp.dpb.cornell.edu/academic-program-regulation/approval-and-registration/cip-codes`、`https://international.globallearning.cornell.edu/employment-and-taxes/opt-stem-extension` 三页正文，均无该事实（`research/stem-cip` 分支 2026-08-15 逐页复核结论）——这不是「没查」，是「查过没有」，故用**官网未列**后缀
...
- `✓ https://www.cs.cornell.edu/master-engineering-computer-science` 学位要求里 `Three to six (3-6) of the 30 credits must be earned as project credit` —— 是 **project，不是 thesis**

> ✅ **在切片内**，本行不用动。⚠️ 它成立是因为**页上写了**，不是因为「MEng 通常是 coursework」。
