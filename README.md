...
按产品流程的自然阶段划分，一个阶段对应一个技能——每个技能具体做什么、怎么做，见它自己的技能说明。**目前仓库里实际落盘的是 `apply-us-master`、`pick-programs` 与 `write-essays` 三个**；`assemble-packet` 随它自己的实现票落盘后，会自动出现在同一个已安装条目里，不需要重新安装。
