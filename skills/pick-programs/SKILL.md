...
- `硬约束` holds procedural restrictions such as "only one program per school per semester" or "no one but the applicant may submit on their behalf"; the only home for cost-type facts is `费用与资格`, and `硬约束` does not take them.
...
Underneath that fact line, follow with a judgment line opening with `- → ` that spells out how this skill intends to handle it, and states honestly whether this is a presumption: when the official text is vague or simply silent, apply a conservative baseline — grammar/spelling-level assistance is fine, substantive ghostwriting is not, whole-paragraph translation is not — and say explicitly that this is a presumption, not the school's own written word; within the same school, if this program's own college has no findable policy but another, more tightly organized unit at the same school has already turned up a stricter policy, that conclusion may be borrowed as a conservative default — but state just as clearly that this is a borrowed presumption, not this channel's own original text.
...
| `口径不清` | Found it, but the year, cohort, or meaning is unclear | Pin down the basis before using it |
